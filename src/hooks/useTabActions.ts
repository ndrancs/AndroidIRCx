...
export const useTabActions = (params: UseTabActionsParams) => {
  ...
  const handleTabPress = useCallback(
    (tabId: string) => {
      ...
      if (tab) {
        ...
        setTabs(prev =>
          prev.map(t => (t.id === tabId ? { ...t, hasActivity: false } : t)),
        );
      }
  ...
  const closeAllChannelsAndQueries = useCallback(
    async (networkId: string) => {
      ...
      await tabService.saveTabs(
        networkId,
        remaining.filter(tab => tab.networkId === networkId),
      );

      if (toClose.some(tab => tab.id === activeTabId)) {
        const serverTab = remaining.find(
          tab => tab.networkId === networkId && tab.type === 'server',
        );
        if (serverTab) {
