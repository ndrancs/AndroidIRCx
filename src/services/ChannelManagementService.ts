...
export class ChannelManagementService {
  ...
  initialize(): void {
    ...
    this.ircService.addRawMessage(
      '*** ChannelManagementService initialized',
      'debug',
    );
    this.ircService.on(
      'topic',
      (channel: string, topic: string, setBy: string) =>
        this.updateTopic(channel, topic, setBy),
    );
    this.ircService.on(
      'channelMode',
      (channel: string, modeString: string, modeParams: string[]) =>
        this.updateModes(channel, modeString, modeParams),
    );
    this.ircService.on('clear-channel', (channel: string) =>
    ...
    this.ircService.on(
      ...
      (
        ...
        if (numeric === 324) {
          // RPL_CHANNELMODEIS - Response to MODE #channel query
          ...
          const topicTime = params[3]
            ? parseInt(params[3], 10) * 1000
            : undefined;
          ...
          // RPL_INVITELIST - Invite list entry
          const channel = params[1] || '';
          const mask = params[2] || '';
          if (channel && mask) {
          ...
          // RPL_EXCEPTLIST - Exception list entry
          const channel = params[1] || '';
          const mask = params[2] || '';
          if (channel && mask) {
          ...
          // RPL_ENDOFEXCEPTLIST - End of exception list
          const channel = params[1] || '';
          if (channel) {
  ...
  setChannelMode(channel: string, mode: string, param?: string): void {
    const modeString =
      mode.startsWith('+') || mode.startsWith('-') ? mode : `+${mode}`;
    const command = param
   ...
   */
  onChannelInfoChange(
    callback: (channel: string, info: ChannelInfo) => void,
  ): () => void {
    this.channelInfoListeners.push(callback);
    return () => {
      this.channelInfoListeners = this.channelInfoListeners.filter(
        cb => cb !== callback,
      );
    };
  }
  ...
  getModeString(channel: string): string {
    ...
    if (m.exceptionList && m.exceptionList.length > 0)
      modes.push(`e(${m.exceptionList.length})`);
    if (m.inviteList && m.inviteList.length > 0)
      modes.push(`I(${m.inviteList.length})`);
