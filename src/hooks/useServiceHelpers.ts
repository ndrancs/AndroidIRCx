...
export const useServiceHelpers = (params: UseServiceHelpersParams) => {
    ...
    (networkId: string, text: string) => {
      ...
      if (existingServer) {
        const updated = currentTabs.map(t =>
          t.id === serverId ? { ...t, messages: [...t.messages, msg] } : t,
        );
  ...
  const getActiveUserManagementService = useCallback(() => {
    ...
    return activeConnection
      ? activeConnection.userManagementService
  ...
  const getActiveConnectionQualityService = useCallback(() => {
    const activeConnection = connectionManager.getActiveConnection();
    return activeConnection
      ? activeConnection.connectionQualityService
  ...
  const getActiveChannelManagementService = useCallback(() => {
    const activeConnection = connectionManager.getActiveConnection();
    return activeConnection
      ? activeConnection.channelManagementService
      : channelManagementService;
  }, []);

  const normalizeNetworkId = useCallback(
    (id: string) => id.replace(/\s+\(\d+\)$/, ''),
    [],
  );

  const getNetworkConfigForId = useCallback(
