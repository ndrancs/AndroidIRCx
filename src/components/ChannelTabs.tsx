    ...
    return (
      ...
      >
        <View style={styles.tabContent}>
          {showEncryptionIndicators &&
            (tab.type === 'query' || tab.type === 'channel') && (
...
export const ChannelTabs: React.FC<ChannelTabsProps> = React.memo(
  ({
    ...
    const [scrollSwitchTabsEnabled, setScrollSwitchTabsEnabled] =
      useState(false);
    const [scrollSwitchTabsInverse, setScrollSwitchTabsInverse] =
      useState(false);
    const [alwaysEncryptStatus, setAlwaysEncryptStatus] = useState<
    ...
    useEffect(() => {
      ...
      Promise.all(
        ...
        .catch(error => {
          debugLogger.warn(
            'channelTabs',
            'Failed to load always-encrypt tab status',
            error,
          );
        });
    ...
    useEffect(() => {
      const unsubscribe =
        channelEncryptionSettingsService.onAlwaysEncryptChange(
    ...
    const handleScroll = useCallback(
      ...
      [
        activeTabId,
        onTabPress,
        scrollSwitchTabsEnabled,
        scrollSwitchTabsInverse,
        tabs,
      ],
    );

    const renderItem = useCallback<ListRenderItem<ChannelTab>>(
      ({ item }) => (
        <ChannelTabItem
          tab={item}
    ...
    const contentContainerStyle = useMemo(
      () => [styles.scrollContent, isVertical && styles.scrollContentVertical],
      [isVertical, styles],
    );
