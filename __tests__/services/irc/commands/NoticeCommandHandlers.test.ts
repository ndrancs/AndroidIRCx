...
describe('NoticeCommandHandlers', () => {
  ...
  beforeEach(() => {
    ...
    (protectionService.evaluateIncomingMessage as jest.Mock).mockReturnValue(null);
  ...
  describe('handleNOTICE', () => {
    it('adds notice message for regular server notice', () => {
      handleNOTICE(ctx, 'server.net', ['MyNick', 'Welcome to the server!'], Date.now(), {});
      expect(ctx.addMessage).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'notice', text: 'Welcome to the server!' }),
        undefined
      );
    });
    ...
    it('ignores message when user is on ignore list', () => {
      ...
      handleNOTICE(ctx, 'bad!baduser@bad.host', ['MyNick', 'spam'], Date.now(), {});
      expect(ctx.addMessage).not.toHaveBeenCalled();
    });

    it('does not check ignore for server-only prefix (no !)', () => {
      handleNOTICE(ctx, 'server.net', ['MyNick', 'Server notice'], Date.now(), {});
    ...
    it('handles CTCP PING response', () => {
      ...
      handleNOTICE(ctx, 'user!u@host', ['MyNick', `\x01PING ${sentTime}\x01`], Date.now(), {});
    ...
    it('handles generic CTCP response with args', () => {
      ...
      handleNOTICE(ctx, 'user!u@host', ['MyNick', '\x01VERSION IRCClient 1.0\x01'], Date.now(), {});
    ...
    it('handles generic CTCP response without args', () => {
      ...
      handleNOTICE(ctx, 'user!u@host', ['MyNick', '\x01TIME\x01'], Date.now(), {});
      expect(ctx.addMessage).toHaveBeenCalled();
    });

    it('blocks message when protection returns decision', () => {
      (protectionService.evaluateIncomingMessage as jest.Mock).mockReturnValue({ kind: 'flood' });
      handleNOTICE(ctx, 'user!u@host', ['MyNick', 'flooding'], Date.now(), {});
      expect(ctx.handleProtectionBlock).toHaveBeenCalledWith(
        'flood', 'user', 'u', 'host', null
      );
      expect(ctx.addMessage).not.toHaveBeenCalled();
    });

    it('handles channel notice', () => {
      handleNOTICE(ctx, 'user!u@host', ['#channel', 'Channel notice'], Date.now(), {});
      expect(ctx.addMessage).toHaveBeenCalledWith(
    ...
    it('runs blacklist check for user with ! prefix', () => {
      ...
      handleNOTICE(ctx, 'badnick!baduser@bad.host', ['#channel', 'bad notice'], Date.now(), {});
    ...
    it('checks mask from notice for server prefix', () => {
      ...
      mockFindMatchingBlacklistEntry.mockReturnValue(blacklistEntry);
      handleNOTICE(ctx, 'server.net', ['MyNick', 'nick!user@host joined'], Date.now(), {});
      expect(ctx.runBlacklistAction).toHaveBeenCalled();
    });

    it('passes batchTag to addMessage', () => {
      handleNOTICE(ctx, 'server.net', ['MyNick', 'test'], Date.now(), { batchTag: 'batch123' });
      expect(ctx.addMessage).toHaveBeenCalledWith(
    ...
    it('handles protection block for channel notice', () => {
      (protectionService.evaluateIncomingMessage as jest.Mock).mockReturnValue({ kind: 'spam' });
      handleNOTICE(ctx, 'user!u@host', ['#channel', 'spam notice'], Date.now(), {});
      expect(ctx.handleProtectionBlock).toHaveBeenCalledWith(
        'spam', 'user', 'u', 'host', '#channel'
      );
    });
  });
});
