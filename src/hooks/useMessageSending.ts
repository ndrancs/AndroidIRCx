...
export const useMessageSending = (params: UseMessageSendingParams) => {
  ...
  const handleSendMessage = useCallback(
    async (message: string) => {
      ...
      const activeTab =
        currentTabs.find(tab => tab.id === activeTabId) ||
        currentTabs.find(tabItem => tabItem.type === 'server') ||
        currentTabs[0];
      ...
      if (!message.trim().startsWith('/')) {
        const decorEnabled = await settingsService.getSetting(
          'decorEnabled',
          false,
        );
        if (decorEnabled) {
          const decorUseColors = await settingsService.getSetting(
            'decorUseColors',
            true,
          );
          const decorBold = await settingsService.getSetting(
            'decorBold',
            false,
          );
          const decorUnderline = await settingsService.getSetting(
            'decorUnderline',
            false,
          );
          const decorTextStyleId = await settingsService.getSetting(
            'decorTextStyleId',
            '',
          );
          const decorColorStyleId = await settingsService.getSetting(
            'decorColorStyleId',
            '',
          );
          const decorAdornmentId = await settingsService.getSetting(
            'decorAdornmentId',
            '',
          );
      ...
      const processedCommand = await activeCommandService.processCommand(
        outgoingMessage,
        activeTab.name,
      );
      ...
      let commandToSend =
        processedCommand !== null && processedCommand.startsWith('/')
          ? processedCommand
          : outgoingMessage;

      // Run scripting on outgoing commands (aliases / automation)
      const scripted = scriptingService.processOutgoingCommand(commandToSend, {
      ...
      if (normalizedCommand.startsWith('/')) {
        ...
        if (baseCommand === '/ctcp') {
          const isQueryTarget =
            activeTab.type === 'query' ? activeTab.name : null;
          ...
          if (!isConnected) {
            safeAlert(
              ...
              t('Please connect to a server first', {
                _tags: 'screen:app,file:App.tsx,feature:connect',
              }),
            );
            return;
          }
          activeIRCService.sendCTCPRequest(
            ctcpTarget,
            ctcpCommand.toUpperCase(),
            ctcpArgs || undefined,
          );
          return;
        }
        ...
        if (baseCommand === '/dcc') {
          ...
          if (parts.length < 2) {
            reportCommandError(
              t('Usage: /dcc <chat|send> <nick> [path] [port]'),
            );
            return;
          }
          ...
          if (subcommand === 'chat') {
            ...
            const networkId =
              activeTab.networkId || activeIRCService.getNetworkName();
            await dccChatService.initiateChat(
              activeIRCService,
              peerNick,
              networkId,
            );
          ...
          if (subcommand === 'send') {
            ...
            try {
              ...
              activeIRCService.addMessage({
                type: 'notice',
                text: t('DCC SEND offer initiated to {nick}', {
                  nick: peerNick,
                }),
                timestamp: Date.now(),
              });
        ...
        if (activeTab.dccSessionId) {
          ...
          if (activeTab.networkId && activeTab.networkId !== 'Not connected') {
            messageHistoryService
              .saveMessage(dccMessage, activeTab.networkId)
              .catch(err => {
      ...
      if (!isConnected) {
        offlineQueueService.addMessage(
          activeTab.networkId,
          activeTab.name,
          commandToSend,
        );
        ...
        if (activeTab.networkId && activeTab.networkId !== 'Not connected') {
          messageHistoryService
            .saveMessage(pendingMessage, activeTab.networkId)
            .catch(err => {
      ...
      const wantEncrypted = !!(
        activeTab.sendEncrypted && activeTab.isEncrypted
      );
      ...
      if (isPrivateTarget && wantEncrypted && !commandToSend.startsWith('/')) {
        ...
        if (!hasBundle) {
            ...
            prev.map(tab =>
              tab.id === activeTab.id
                ? {
                    ...tab,
                    isEncrypted: false,
                    messages: [...tab.messages, errorMsg],
                  }
                : tab,
            ),
          );
          ...
          if (activeTab.networkId && activeTab.networkId !== 'Not connected') {
            messageHistoryService
              .saveMessage(errorMsg, activeTab.networkId)
              .catch(err => {
        ...
        try {
          const encryptedNetworkId =
            activeTab.networkId || activeIRCService.getNetworkName();
          const payload = await encryptedDMService.encryptForNetwork(
            commandToSend,
            encryptedNetworkId,
            activeTab.name,
          );
          activeIRCService.sendRaw(
            `PRIVMSG ${activeTab.name} :!enc-msg ${JSON.stringify(payload)}`,
          );
          ...
          if (activeTab.networkId && activeTab.networkId !== 'Not connected') {
            messageHistoryService
              .saveMessage(sentMessage, activeTab.networkId)
              .catch(err => {
      ...
      const isChannel =
        activeTab.name.startsWith('#') || activeTab.name.startsWith('&');
      if (isChannel && wantEncrypted && !commandToSend.startsWith('/')) {
        ...
        if (hasKey) {
          try {
            const payload = await channelEncryptionService.encryptMessage(
              commandToSend,
              activeTab.name,
              activeTab.networkId,
            );
            activeIRCService.sendRaw(
              `PRIVMSG ${activeTab.name} :!chanenc-msg ${JSON.stringify(payload)}`,
            );
            const sentMessage: IRCMessage = {
              id: `msg-${Date.now()}-${Math.random()}`,
              type: 'message',
            ...
            if (
              activeTab.networkId &&
              activeTab.networkId !== 'Not connected'
            ) {
              messageHistoryService
                .saveMessage(sentMessage, activeTab.networkId)
                .catch(err => {
                  console.error(
                    'Error saving encrypted channel message to history:',
                    err,
                  );
                });
            }
