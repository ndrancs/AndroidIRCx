...
export function WebRTCCallModal({ activeTab }: WebRTCCallModalProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribeNotification = settingsService.onSettingChange<boolean>(
  ...
  const shouldRenderMinimizedGlobally =
    !showOnlyOnActiveQuery || activeQueryMatchesPeer;
  ...
  const overlayHeight =
    mediaType === 'video' ? overlayWidth * VIDEO_ASPECT_RATIO : 70;
  const maxOverlayX = Math.max(
    OVERLAY_MARGIN,
    window.width - overlayWidth - OVERLAY_MARGIN,
  );
  const maxOverlayY = Math.max(
    OVERLAY_MARGIN,
    window.height - overlayHeight - 120,
  );
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderRelease: (_, gestureState) => {
          ...
          webRtcCallService.snapOverlayToEdge(
            window.width,
            overlayWidth,
            nextY,
          );
      ...
      {phase === 'incoming' ? (
        <>
          <TouchableOpacity
            ...
            onPress={() => webRtcCallService.declineIncomingCall()}
          >
            <Icon name="phone-slash" size={18} color="#fff" />
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.button, styles.answerButton]}
            onPress={() => webRtcCallService.acceptIncomingCall()}
          >
            <Icon
              name={mediaType === 'video' ? 'video' : 'phone'}
              size={18}
              color="#fff"
            />
          </TouchableOpacity>
        ...
        <>
          <TouchableOpacity
            style={styles.button}
            onPress={() => webRtcCallService.toggleMute()}
          >
            <Icon
              name={micMuted ? 'microphone-slash' : 'microphone'}
              size={18}
              color="#fff"
            />
          </TouchableOpacity>
          {mediaType === 'video' && (
            <TouchableOpacity
              style={styles.button}
              onPress={() => webRtcCallService.toggleCamera()}
            >
              <Icon
                name={cameraEnabled ? 'video' : 'video-slash'}
                size={18}
                color="#fff"
              />
            </TouchableOpacity>
          ...
          <TouchableOpacity
            style={[styles.button, styles.dangerButton]}
            onPress={() => webRtcCallService.hangUp()}
          >
            <Icon name="phone-slash" size={18} color="#fff" />
          </TouchableOpacity>
        </>
  ...
  return (
    <>
      <Modal
        ...
        transparent
        animationType="slide"
        onRequestClose={() => webRtcCallService.hangUp()}
      >
            ...
            <View style={styles.header}>
              <View style={styles.headerRow}>
                <Text style={styles.title}>
                  {mediaType === 'video' ? 'Video Call' : 'Audio Call'}
                </Text>
                {canMinimize && (
                  <View style={styles.headerActions}>
                    <TouchableOpacity
                      style={styles.headerButton}
                      onPress={() => webRtcCallService.minimizeCall()}
                    >
              ...
              <Text style={styles.subtitle}>
                {peerNick || 'Unknown peer'} ·{' '}
                {statusText ||
                  (direction === 'incoming' ? 'Incoming call' : 'Connecting')}
              </Text>
              <View style={styles.relayBadge}>
                <Text style={styles.relayText}>
                  {usingRelay ? 'Relay ready' : 'Direct WebRTC'}
                </Text>
              </View>
            </View>
        ...
        <View style={styles.minimizedLayer} pointerEvents="box-none">
          {mediaType === 'video' ? (
            ...
            >
              <View style={styles.videoOverlayTop}>
                ...
                <View style={styles.videoMiniActionGroup}>
                  <TouchableOpacity
                    style={styles.videoMiniButton}
                    onPress={() => webRtcCallService.restoreCall()}
                  >
                    <Icon name="expand" size={12} color="#fff" />
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.videoMiniButton, styles.videoMiniDanger]}
                    onPress={() => webRtcCallService.hangUp()}
                  >
              ...
              >
                {remoteStream ? (
                  ...
                  <View style={styles.videoPlaceholder}>
                    ...
                    <Text style={styles.videoPlaceholderStatus}>
            ...
            >
              ...
              >
                <Icon
                  name={micMuted ? 'microphone-slash' : 'phone'}
                  size={16}
                  color="#fff"
                />
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={[styles.miniAction, styles.miniDanger]}
                onPress={() => webRtcCallService.hangUp()}
              >
      ...
      {visible &&
        minimized &&
        !shouldRenderMinimizedGlobally &&
        showOnlyOnActiveQuery && (
          <View style={styles.minimizedLayer} pointerEvents="box-none">
            <TouchableOpacity
              style={styles.scopedBadge}
              activeOpacity={0.9}
              onPress={() => webRtcCallService.restoreCall()}
            >
