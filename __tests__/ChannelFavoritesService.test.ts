...
describe('ChannelFavoritesService', () => {
  ...
  it('adds, updates, and removes favorites', async () => {
    ...
    expect(channelFavoritesService.getFavorites('net1')[0]).toMatchObject({
      key: 'key',
      autoJoin: true,
    });
  ...
  it('toggles autoJoin and key updates', async () => {
    ...
    expect(
      channelFavoritesService.getAutoJoinChannels('net')[0]?.autoJoin,
    ).toBe(true);
  ...
  it('initializes from storage and returns auto-join only entries', async () => {
    const seed = new Map([
      [
        'net',
        [
          { name: '#a', network: 'net', autoJoin: true, addedAt: 1 },
          { name: '#b', network: 'net', autoJoin: false, addedAt: 2 },
        ],
      ],
    ]);
    (AsyncStorage as any).__STORE.set(
      '@AndroidIRCX:channelFavorites',
      JSON.stringify(Object.fromEntries(seed)),
    );
  ...
  it('gracefully handles toggles and key updates for missing favorites', async () => {
    ...
    await expect(
      channelFavoritesService.setAutoJoin('net', '#missing', true),
    ).resolves.not.toThrow();
    await expect(
      channelFavoritesService.updateFavoriteKey('net', '#missing', 'k'),
    ).resolves.not.toThrow();
