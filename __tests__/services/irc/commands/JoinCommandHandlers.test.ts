...
import {
  joinCommandHandlers,
  handleJOIN,
} from '../../../../src/services/irc/commands/JoinCommandHandlers';
...
describe('JoinCommandHandlers', () => {
  const createMockContext = (): Partial<CommandHandlerContext> => ({
    ...
    extractNick: jest
      .fn()
      .mockImplementation((prefix: string) => prefix.split('!')[0]),
  ...
  describe('handleJOIN', () => {
    ...
    it('should handle join without extended join capability', () => {
      ...
      handleJOIN(
        ctx,
        'SimpleUser!~user@host.com',
        ['#general', 'account_name'],
        Date.now(),
      );
    ...
    it('should handle extended join with account', () => {
      ...
      handleJOIN(
        ctx,
        'RegisteredUser!~user@host.com',
        ['#general', 'registered_account', ':Real Name'],
        Date.now(),
      );
    ...
    it('should handle extended join with * account (no account)', () => {
      ...
      handleJOIN(
        ctx,
        'UnregisteredUser!~user@host.com',
        ['#general', '*', ':Real Name'],
        Date.now(),
      );
    ...
    it('should update existing user account if already in channel', () => {
      ...
      handleJOIN(
        ctx,
        'ExistingUser!~user@host.com',
        ['#general', 'new_account', ':Real Name'],
        Date.now(),
      );
    ...
    it('should extract username and hostname from prefix', () => {
      ...
      handleJOIN(
        ctx,
        'JoiningUser!~username@hostname.com',
        ['#general'],
        Date.now(),
      );
