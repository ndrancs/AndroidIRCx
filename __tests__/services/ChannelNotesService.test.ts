...
import {
  channelNotesService,
  ChannelLogEntry,
} from '../../src/services/ChannelNotesService';
...
describe('ChannelNotesService', () => {
  ...
  describe('setNote / getNote', () => {
    ...
    it('should handle multiple networks independently', async () => {
      ...
      expect(await channelNotesService.getNote('freenode', '#general')).toBe(
        'Freenode note',
      );
      expect(await channelNotesService.getNote('libera', '#general')).toBe(
        'Libera note',
      );
    });
    ...
    it('should handle multiple channels on same network', async () => {
      ...
      expect(await channelNotesService.getNote('freenode', '#random')).toBe(
        'Random note',
      );
    });
  ...
  describe('setBookmarked / isBookmarked', () => {
    it('should set and check bookmark', async () => {
      ...
      const isBookmarked = await channelNotesService.isBookmarked(
        'freenode',
        '#general',
      );
    ...
    it('should handle multiple bookmarks', async () => {
      ...
      expect(
        await channelNotesService.isBookmarked('freenode', '#general'),
      ).toBe(true);
      expect(
        await channelNotesService.isBookmarked('freenode', '#random'),
      ).toBe(true);
      expect(await channelNotesService.isBookmarked('libera', '#general')).toBe(
        true,
      );
    });
  });
  ...
  describe('addLogEntry / getLog', () => {
    ...
    it('should handle multiple channel logs independently', async () => {
      ...
      const generalLog = await channelNotesService.getLog(
        'freenode',
        '#general',
      );
  ...
  describe('ensureLoaded', () => {
    it('should load data from storage on first access', async () => {
      ...
      mockStorage.channelNotes = JSON.stringify({
        'freenode::#general': 'Loaded note',
      });
    ...
    it('should handle storage errors gracefully', async () => {
      ...
      // Should not throw
      await expect(
        channelNotesService.getNote('freenode', '#general'),
      ).resolves.toBe('');
    });
