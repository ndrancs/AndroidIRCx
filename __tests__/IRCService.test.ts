...
describe.skip('IRCService MOTD Handling (smoke, mocked connect)', () => {
  ...
  beforeEach(() => {
    ...
    (TcpSocket.createConnection as jest.Mock).mockImplementation(
      (options, callback) => {
        callback();
        mockSocket.on.mock.calls.forEach(
    ...
    (TcpSocket.connectTLS as jest.Mock).mockImplementation(
      (options, callback) => {
        callback();
        mockSocket.on.mock.calls.forEach(
          ([event, handler]: [string, Function]) => {
  ...
  it('should process RPL_MOTDSTART, RPL_MOTD, and RPL_ENDOFMOTD', async () => {
    ...
    triggerData(
      ':mock.irc.server 001 testnick :Welcome to the Mock IRC Network testnick!testuser@mock.irc.server\r\n',
    );
    triggerData(
      ':mock.irc.server 375 testnick :- mock.irc.server Message of the Day -\r\n',
    );
    triggerData(
      ':mock.irc.server 372 testnick :- This is line 1 of the MOTD\r\n',
    );
    triggerData(
      ':mock.irc.server 372 testnick :- This is line 2 of the MOTD\r\n',
    );
    ...
    const welcomeMsg = rawMessages.find(msg =>
      msg.text.includes('Welcome to the Mock IRC Network'),
    );
    expect(welcomeMsg).toBeDefined();
    expect(welcomeMsg?.isRaw).toBe(true);

    const motdStart = rawMessages.find(msg =>
      msg.text.includes('Message of the Day -'),
    );
  ...
  it('should process ERR_NOMOTD when no MOTD is available', async () => {
    ...
    const noMotdMsg = rawMessages.find(msg =>
      msg.text.includes('No Message of the Day.'),
    );
