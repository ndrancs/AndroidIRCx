...
describe('UserActivityService', () => {
  ...
  describe('recordEvent', () => {
    ...
    it('should record event with channel context', () => {
      userActivityService.recordEvent('TestUser', 'freenode', 'JOIN', {
        channel: '#general',
      });
    ...
    it('should distinguish users by network', () => {
      ...
      const freenodeActivity = userActivityService.getActivity(
        'TestUser',
        'freenode',
      );
      const liberaActivity = userActivityService.getActivity(
        'TestUser',
        'libera',
      );
  ...
  describe('getActivity', () => {
    it('should return undefined for unknown user', () => {
      const activity = userActivityService.getActivity(
        'UnknownUser',
        'freenode',
      );
  ...
  describe('clearNetwork', () => {
    it('should clear all activities for a network', () => {
      ...
      expect(
        userActivityService.getActivity('User1', 'freenode'),
      ).toBeUndefined();
      expect(
        userActivityService.getActivity('User2', 'freenode'),
      ).toBeUndefined();
    ...
    it('should handle clearing empty network', () => {
      expect(() =>
        userActivityService.clearNetwork('nonexistent'),
      ).not.toThrow();
    });
    ...
    it('should only clear exact network matches', () => {
      ...
      expect(
        userActivityService.getActivity('User2', 'freenode-backup'),
      ).toBeDefined();
    });
  });
});
