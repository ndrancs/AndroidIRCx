...
export const useAutoJoinChannels = (params: UseAutoJoinChannelsParams) => {
  ...
  const motdFallbackTimersRef = useRef<
  ...
  useEffect(() => {
    return () => {
      Object.values(motdFallbackTimersRef.current).forEach(timer =>
        clearTimeout(timer),
      );
  ...
  useEffect(() => {
    const handleAutoJoin = async () => {
      ...
      if (
        ...
        const exactMatch =
          networks.find(n => n.id === activeNetId || n.name === activeNetId) ||
          null;
        ...
        const channelsToJoin = Array.from(
          new Set([...favoriteNames, ...autoJoin]),
        );
