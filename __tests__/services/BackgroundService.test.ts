...
jest.mock('../../src/services/NotificationService', () => ({
  notificationService: {
    ...
    showMessageNotification: (...args: any[]) => mockShowMessageNotification(...args),
    ...
    updateNetworkPreferences: (...args: any[]) => mockUpdateNetworkPreferences(...args),
...
jest.mock('react-native-battery-optimization-check', () => ({
  ...
  OpenOptimizationSettings: (...args: any[]) => mockOpenOptimizationSettings(...args),
}));
...
jest.mock('../../src/i18n/transifex', () => ({
  tx: {
    t: jest.fn((key: string, params?: Record<string, any>) => {
      ...
      return key.replace('{channel}', params.channel || '').replace('{count}', String(params.count || ''));
    }),
  },
}));
...
describe('BackgroundService', () => {
  ...
  it('initializes app state listener and showRaw setting watcher', async () => {
    let appStateCb: any;
    mockAppStateAddEventListener.mockImplementation((_event: string, cb: any) => {
      appStateCb = cb;
    ...
    expect(mockOnSettingChange).toHaveBeenCalledWith('showRawCommands', expect.any(Function));
  ...
  it('attaches listener in background, persists message and throttles notifications', async () => {
    ...
    mockGetAllConnections.mockReturnValue([{ networkId: 'netA', ircService: irc }]);
    ...
    const msg = { type: 'message', from: 'alice', text: 'hello', channel: '#chat', network: 'NetA' };
    ...
    expect((backgroundService as any).notificationQueue.get('NetA:#chat')?.length).toBe(1);
  });

  it('flushes queued notifications via processNotificationQueue', async () => {
    (backgroundService as any).notificationQueue = new Map([
      ['NetA:#help', [{ type: 'message', text: 'last', channel: '#help', network: 'NetA' }]],
    ]);
  ...
  it('proxies notification preference updates and getter', async () => {
    ...
    await backgroundService.updateChannelNotificationConfig('#a', { notifyOnAllMessages: true });
    await backgroundService.updateNetworkNotificationConfig('NetA', { doNotDisturb: true });

    expect(mockUpdatePreferences).toHaveBeenCalledWith({ enabled: false });
    expect(mockUpdateChannelPreferences).toHaveBeenCalledWith('#a', { notifyOnAllMessages: true });
    expect(mockUpdateNetworkPreferences).toHaveBeenCalledWith('NetA', { doNotDisturb: true });
