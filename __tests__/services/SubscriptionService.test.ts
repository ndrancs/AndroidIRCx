...
describe('SubscriptionService', () => {
  ...
  it('covers metadata/password lookups and account helper queries', async () => {
    (subscriptionService as any).accounts = [
      {
        id: 'h1', zncUsername: 'Alice', zncPassword: 'pwA', status: 'active', provisioningStatus: 'ready',
        expiresAt: null, purchaseToken: 'tokA', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: 'n1', assignedServerId: 's1', createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
      {
        id: 'h2', zncUsername: 'Bob', zncPassword: null, status: 'expired', provisioningStatus: 'provisioning',
        expiresAt: '2000-01-01T00:00:00.000Z', purchaseToken: '', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: null, assignedServerId: null, createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
    ];

    const meta = subscriptionService.getAccountsMetadata(new Map([['n1', 'Net One']]));
    ...
    mockGetSetting.mockImplementation(async (k: string) => k === 'biometricPasswordLock');
    mockSecure.getSecret.mockResolvedValueOnce('storedPw');
    await expect(subscriptionService.getAccountPassword('h2')).resolves.toBe('storedPw');

    mockGetSetting.mockResolvedValueOnce(false);
    await expect(subscriptionService.getAccountPassword('h1')).resolves.toBe('pwA');
  });
  ...
  it('covers initialize idempotency and load/save error branches', async () => {
    ...
    mockGetSetting.mockImplementation(async (k: string) => k === 'biometricPasswordLock');
    const prepared = await (subscriptionService as any).prepareAccountsForStorage([
      {
        id: 'sec-1', zncUsername: 'sec', zncPassword: 'pw', status: 'active', provisioningStatus: 'ready',
        expiresAt: null, purchaseToken: 'pt', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: null, assignedServerId: null, createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
    ], true);
  ...
  it('covers getPurchaseTokens for secure-storage and fallback branches', async () => {
    mockGetSetting.mockImplementation(async (k: string) => k === 'biometricPasswordLock');
    await AsyncStorage.setItem(ZNC_STORAGE_KEYS.TOKENS, JSON.stringify([{ accountId: 'a1' }, { accountId: 'a2' }]));
    mockSecure.getSecret.mockResolvedValueOnce('tok1').mockResolvedValueOnce(null);
    await expect(subscriptionService.getPurchaseTokens()).resolves.toEqual(['tok1']);

    mockGetSetting.mockResolvedValueOnce(false);
    await AsyncStorage.setItem(ZNC_STORAGE_KEYS.TOKENS, JSON.stringify([{ token: 'plain1' }, { token: 'plain2' }]));
    await expect(subscriptionService.getPurchaseTokens()).resolves.toEqual(['plain1', 'plain2']);

    await AsyncStorage.removeItem(ZNC_STORAGE_KEYS.TOKENS);
    mockGetSetting.mockImplementation(async () => false);
    (subscriptionService as any).accounts = [
      {
        id: 'f1', zncUsername: 'u', zncPassword: null, status: 'active', provisioningStatus: 'ready',
        expiresAt: null, purchaseToken: 'fallback-tok', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: null, assignedServerId: null, createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
    ];
    await expect(subscriptionService.getPurchaseTokens()).resolves.toEqual(['fallback-tok']);
  });

  it('covers refreshAllAccounts and assign/unassign error handling', async () => {
    (subscriptionService as any).accounts = [
      {
        id: 'r1', zncUsername: 'u1', zncPassword: null, status: 'expired', provisioningStatus: 'ready',
        expiresAt: '2999-01-01T00:00:00.000Z', purchaseToken: 't1', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: null, assignedServerId: null, createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
      {
        id: 'r2', zncUsername: 'u2', zncPassword: null, status: 'active', provisioningStatus: 'ready',
        expiresAt: '2000-01-01T00:00:00.000Z', purchaseToken: 't2', subscriptionId: ZNC_PRODUCT_ID,
        assignedNetworkId: null, assignedServerId: null, createdAt: new Date().toISOString(), lastRefreshedAt: null,
      },
    ];
    ...
    await expect(subscriptionService.assignToNetwork('missing', 'n', 's')).rejects.toThrow('Account not found');
    await expect(subscriptionService.unassignFromNetwork('missing')).rejects.toThrow('Account not found');
  });

  it('covers restorePurchases individual fallback flow and failures', async () => {
    (global as any).fetch
      .mockResolvedValueOnce({ ok: true, status: 200, json: async () => ({ accounts: [] }) }) // batch no results
      .mockResolvedValueOnce({
        ok: true, status: 200, json: async () => ({
          id: 'i1', znc_username: 'ind1', znc_password: 'pw1', status: 'active', znc_status: 'ready', expires_at: null,
        }),
      })
      .mockRejectedValueOnce(new Error('token failed'));

    const out = await subscriptionService.restorePurchases(['tok-i1', 'tok-bad']);
  ...
  it('covers register/update and apiCall error parsing branches', async () => {
    ...
    (global as any).fetch.mockResolvedValueOnce({
      ok: true, status: 200, json: async () => ({
        id: 'upd-1', status: 'active', expires_at: null, znc_username: 'upd', znc_password: '', znc_status: null,
      }),
    });
    await subscriptionService.registerZncSubscription({
      purchaseToken: 'pt-1', subscriptionId: ZNC_PRODUCT_ID, zncUsername: 'upd',
    });
    ...
    (global as any).fetch.mockResolvedValueOnce({
      ok: true, status: 200, json: async () => ({
        id: 'upd-1', status: 'active', expires_at: null, znc_username: 'upd', znc_password: 'newpw', znc_status: 'ready',
      }),
    });
    const updated = await subscriptionService.registerZncSubscription({
      purchaseToken: 'pt-2', subscriptionId: ZNC_PRODUCT_ID, zncUsername: 'upd',
    });
    ...
    await expect(subscriptionService.checkUsernameAvailability('x')).resolves.toBe(true);
    ...
    await expect(subscriptionService.checkUsernameAvailability('y')).resolves.toBe(true);
    ...
    await expect(subscriptionService.checkUsernameAvailability('z')).resolves.toBeUndefined();
  });
});
