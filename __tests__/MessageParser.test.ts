...
describe('MessageParser media detection', () => {
  it('detects common video extensions', () => {
    ...
    expect(isVideoUrl('https://cdn.example.com/path/clip.MOV?token=1')).toBe(
      true,
    );
  ...
  it('detects image extensions and urls', () => {
    ...
    expect(isImageUrl('https://example.com/assets/icon.PNG?cache=1')).toBe(
      true,
    );
  ...
  it('detects downloadable files by extension', () => {
    ...
    expect(isDownloadableFileUrl('https://example.com/archive.tar.gz')).toBe(
      true,
    );
