...
export function buildClientFirstMessage(
  ...
  const message = base64Encode(
    new Uint8Array([
      ...gs2HeaderBytes,
      ...stringToBytes(clientFirstMessageBare),
    ]),
  );
...
export function parseServerFirstMessage(
  ...
  try {
    const serverFirstMessage = bytesToString(
      base64Decode(serverFirstMessageB64),
    );
    ...
    return {
      success: false,
      error: `Failed to parse server-first-message: ${e}`,
    };
  }
}
...
export async function buildClientFinalMessage(
  ...
  const clientSignature = await hmacSha256(
    storedKey,
    stringToBytes(authMessage),
  );
...
export function verifyServerFinalMessage(
  ...
  try {
    const serverFinalMessage = bytesToString(
      base64Decode(serverFinalMessageB64),
    );
    ...
    return {
      success: false,
      error: `Failed to verify server-final-message: ${e}`,
    };
  }
}
...
export class ScramAuthService {
   ...
   */
  processServerFirst(serverFirstMessageB64: string): {
    success: boolean;
    error?: string;
  } {
    if (!this.state) {
  ...
  async buildClientFinal(password: string): Promise<string> {
    ...
    const { message, state } = await buildClientFinalMessage(
      this.state,
      password,
    );
    this.state = state;
  ...
  private async calculateServerSignature(password: string): Promise<void> {
    ...
    const serverKey = await hmacSha256(
      saltedPassword,
      stringToBytes('Server Key'),
    );

    // Calculate ServerSignature
    this.serverSignature = await hmacSha256(
      serverKey,
      stringToBytes(this.state.authMessage),
    );
  }
   ...
   * Verify server-final-message
   */
  verifyServerFinal(serverFinalMessageB64: string): {
    success: boolean;
    error?: string;
  } {
    ...
    return verifyServerFinalMessage(
      this.state,
      serverFinalMessageB64,
