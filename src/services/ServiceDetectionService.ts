...
const IRCD_VERSION_PATTERNS: Array<{
  pattern: RegExp;
  type: IRCdType;
  confidence: number;
}> = [
  ...
  // ircd-hybrid
  { pattern: /ircd-hybrid|hybrid/i, type: 'hybrid', confidence: 0.9 },
...
const IRCD_ISUPPORT_PATTERNS: Array<{
  tokens: string[];
  type: IRCdType;
  confidence: number;
}> = [
  // UnrealIRCd specific
  {
    tokens: ['NICKCHARS', 'NICKIP', 'ESVID'],
    type: 'unrealircd',
    confidence: 0.9,
  },
  {
    tokens: ['MAXLIST=b:100,e:100,I:100'],
    type: 'unrealircd',
    confidence: 0.7,
  },

  // InspIRCd specific
  { tokens: ['NICKMAXLEN'], type: 'inspircd', confidence: 0.8 },
  { tokens: ['MAXMODES=20'], type: 'inspircd', confidence: 0.6 },
  ...
  // Hybrid specific
  { tokens: ['KNOCK', 'CALLERID=g'], type: 'hybrid', confidence: 0.7 },

  // ngIRCd specific (minimal tokens)
  { tokens: ['MAXCHANNELSPERUSER'], type: 'ngircd', confidence: 0.6 },
];
...
export class ServiceDetectionService {
  ...
  private detectionCallbacks: Array<
    (networkId: string, result: DetectionResult) => void
  > = [];
   ...
   */
  public onDetection(
    callback: (networkId: string, result: DetectionResult) => void,
  ): () => void {
    ...
    for (const config of Object.values(allConfigs)) {
      if (config.services) {
        for (const service of Object.values(config.services)) {
          ...
          if (
            service.aliases?.some((a: string) => a.toLowerCase() === lowerNick)
          ) {
            return true;
          }
        }
  ...
  private detectServicesFromToken(
  ...
  private identifyServiceFromMessage(
    nick: string,
    message: string,
  ): string | undefined {
    ...
    if (lowerMsg.includes('nickname') && lowerMsg.includes('register'))
      return 'nickserv';
    if (lowerMsg.includes('channel') && lowerMsg.includes('register'))
      return 'chanserv';
    if (lowerMsg.includes('vhost') || lowerMsg.includes('virtual host'))
      return 'hostserv';
    if (lowerMsg.includes('memo') || lowerMsg.includes('message'))
      return 'memoserv';
  ...
  private detectFromNetworkName(networkName: string): {
    serviceType: IRCServiceType;
    ircdType: IRCdType;
    confidence: number;
  } | null {
  ...
  private detectFromISupport(tokens: Map<string, string>): {
    serviceType: IRCServiceType;
    ircdType: IRCdType;
    confidence: number;
  } | null {
  ...
  private detectFromVersion(version: string): {
    serviceType: IRCServiceType;
    ircdType: IRCdType;
    confidence: number;
  } | null {
  ...
  private detectFromServices(services: Map<string, string>): {
    serviceType: IRCServiceType;
    ircdType: IRCdType;
    confidence: number;
  } | null {
    ...
    if (
      serviceNicks.includes('x') ||
      serviceNicks.some(n => n.includes('x@'))
    ) {
      return {
        serviceType: 'undernet',
        ircdType: 'charybdis',
    ...
    if (
      serviceNicks.includes('q') ||
      serviceNicks.some(n => n.includes('q@'))
    ) {
      return { serviceType: 'quakenet', ircdType: 'unknown', confidence: 0.9 };
    }
    ...
    if (hasChanServ && hasNickServ) {
      ...
      return { serviceType: 'anope', ircdType: 'unrealircd', confidence: 0.6 };
    }
