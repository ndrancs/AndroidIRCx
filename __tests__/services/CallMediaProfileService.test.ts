...
describe('CallMediaProfileService', () => {
  ...
  it('sanitizes custom TURN config (dedupe + invalid URL removal + trim)', async () => {
    mockMediaSettingsService.getCallTurnServerConfig.mockResolvedValue({
      enabled: true,
      urls: [
        '  turn:turn.example.org:3478?transport=udp  ',
        'turn:turn.example.org:3478?transport=udp',
        'http://invalid.example.org',
        'turns:turn.example.org:5349?transport=tcp',
      ],
      username: '  relay-user  ',
      credential: 'relay-pass',
  ...
  it('falls back to default STUN list when loading settings throws', async () => {
    ...
    expect(rtcConfig.relayEnabled).toBe(false);
  ...
  it('throws when relay is active but purchase token is missing', async () => {
    mockPrivacyRelayService.getRelayAccessState.mockReturnValue({
      hasSubscription: true,
      requiresBackendCredentials: true,
      relayServer: {
  ...
  it('clamps disallowed quality to profile default and exposes allowed qualities', () => {
    const allowed = callMediaProfileService.getAllowedVideoQualities();
    const clamped = callMediaProfileService.clampVideoQuality('1080p');
    const preset = callMediaProfileService.getVideoPreset('1080p');

    expect(allowed).toEqual(['480p', '720p']);
    expect(clamped).toBe('480p');
    expect(preset.quality).toBe('480p');
  });
});
